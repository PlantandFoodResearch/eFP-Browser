...
def mafft_align(fasta_file):
    ...
    outfile = tempfile.mkstemp(prefix="mafft_", dir=efpConfig.outputDir)
    ...
    new_outfile = tempfile.mkstemp(prefix="mafft_", dir=efpConfig.outputDir)
    ...
    if match is not None:
        mafft_relPath = '%s/%s' % (efpConfig.outputDir, fileName)
...
def mview_align(mafft_align):
    ...
    outfile = tempfile.mkstemp(prefix="mview_", suffix=".html", dir=efpConfig.outputDir)
    outputr = file(outfile[1], "w")
    retval = subprocess.call(cmd, 0, None, None, outputr, subprocess.STDOUT)
    ...
    mview_relPath = None
    if match is not None:
        mview_relPath = '%s/%s'%(efpConfig.outputDir, fileName)
...
class View:
            ...
            for tissue in group.tissues:
                ...
                if gene2 != None:
                    ...
                    if control == 0 or control2 == 0:
                        ...
                        if ( signal != 0 and signal2 != 0 ): # needed to check for zero here too: bw 2011 PFR remake
                                #print >> sys.stderr, "s1: %s, c1:%s, s2:%s, c2:%s" % (signal, control, signal2, control2)
        ...
        for group in self.groups:
            ...
            for tissue in group.tissues:
                ...
                for coords in tissue.coords:
                    # REMOVED TISSUE LINKING # out += '<area shape="polygon" coords="%s" title="%s \n%s" href="%s">\n' % (coords, tissue.name, sigString, tissue.url) 
                    out += '<area shape="polygon" coords="%s" title="%s \n%s">\n' % (coords, tissue.name, sigString) 
    ...
    def renderComparison(self, gene1, gene2, threshold=0.0):
        ...
        for group in self.groups:
            ...
            for tissue in group.tissues:
                ...
                # Added checks for zero when calculating ratios
    ...
    def drawImage(self, mode, maxSignalInDatasource, viewMaxSignal1, viewMaxSignal2, gene1, gene2, img):
        ...
        files = glob.glob("%s/*" % (efpConfig.outputDir))
        if len(files) > filecounter:
            os.system("rm -f %s/*" % (efpConfig.outputDir))
        # Create a named temporary file with global read permissions
        outfile = tempfile.mkstemp(suffix='.png', prefix='efp-', dir=efpConfig.outputDir)
    ...
    def getTissueSignal(self, gene, sample, control = 0):
        ...
        row = cursor.fetchone()
        # signal = None
        signal = 0.0 # was signal = None, but that requires 'zero signals' for sequences which are not 'present in the sample'
        # so i changed it to default to zero if no signal is found 
        if row != None:
            signal = row[0]
        cursor.close()
        return signal
    ...
    def connect (self):
        try:
            self.conn = MySQLdb.connect (host = efpConfig.DB_HOST, port = efpConfig.DB_PORT, user = efpConfig.DB_USER, passwd = efpConfig.DB_PASSWD, db = self.database)
        except MySQLdb.Error, e:
            print "Error %d: %s" % (e.args[0], e.args[1])

    def createGene(self, geneId):
        return efpDb.Gene(geneId)
...
class SpecimenHandler(handler.ContentHandler):
    def __init__(self, specimen):
        self.spec = specimen
        #testing_efp.write("specimen = %s\n"%(specimen))
        self.ctrlSample = ""
        self.sampleDict = {}
    ...
    def startElement(self, name, attrs):
        if name == 'view':
            ...
            if(viewClass == None):
                viewClass = "View"
            self.sampleDict = {}
            exec "self.currentView = %s(attrs.get('name'), attrs.get('db'), attrs.get('dbGroup'), '%s/' + attrs.get('img'))" % (viewClass, efpConfig.dataDir)

        if name == 'coords':
            graph = (attrs.get('graphX'), attrs.get('graphY'), attrs.get('graphWidth'), attrs.get('graphHeight'))
            legend = (int(attrs.get('legendX')), int(attrs.get('legendY')))
            self.currentView.addGraphCoords(graph)
            self.currentView.addLegendCoords(legend)

        if name == 'extra':
            e = Extra(attrs.get('name'), attrs.get('link'), attrs.get("parameters"), attrs.get('coords'), attrs.get('check'), attrs.get('checkColumn'))
            self.currentView.addExtra(e)
        
        if name == 'group':
            self.currentGroup = Group(attrs.get('name'))
        
        if name == 'control':
            sampleName = attrs.get('sample')
        ...
        if name == 'tissue':
            t = Tissue(attrs.get('name'), attrs.get('colorKey'))
